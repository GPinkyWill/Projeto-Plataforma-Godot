...
func move_platform():
	...
	var platform_tween = create_tween().set_loops().set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
	platform_tween.tween_property(self,"follow",move_direction,duration).set_delay(WAIT_DURATION + _delay)
	platform_tween.tween_property(self,"follow",Vector2.ZERO,duration).set_delay(duration + _delay + WAIT_DURATION)
